...
\paragraph{The Disposable contract} At the lowest level, the framework defines
a functional interface \kotlin{Disposable} which provides a uniform contract
for any entity holding resources that must be explicitly released. Core domain
entities, including \kotlin{Node} and \kotlin{Reaction}, implement this
interface. When a node is disposed, it recursively dispose every reaction
attached to it and every reaction in turn dispose their conditions. This
explicit cascading cleanup is the first line of defense against leaks.

\paragraph{Lifecycle-Aware components} While manual disposal is effective, it
is error-prone if relied upon for every individual subscription. To automate
this, the concept of \emph{Lifecycle-aware components} is introduced. The
architecture revolves around three key abstractions:
\begin{itemize}
	\item \kotlin{LifecycleState} is a simple enumeration defining the
	      valid states of a component. At the time of writing, the states
	      \kotlin{INITIALIZED}, \kotlin{STARTED} and \kotlin{DESTROYED}
	      are defined; however, further works could enrich this set state
	      to include finer-grained states both for better debugging
	      purposes or better subscription's state management (see the
	      \textsc{AndroidX} implementation for reference);
...
%
The \kotlin{bindTo} (defined as \Cref{lst:bindTo}) method performs three critical
functions:
\begin{enumerate}
...
cleanly severs all its incoming and outgoing reactive links, solving the memory
leak problem by design rather than by convention.

\section{Alchemist Reactive Engine Integration}
