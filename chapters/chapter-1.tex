...
This problem is rigorously formalized in terms of complex biochemical systems
...
full scan. However, this approach historically relies on the assumption of
a static structure. In standard NRM implementations, the
dependency graph is derived from the stoichiometry of the chemical equations
and remains immutable throughout the simulation.

\subsection{REVIEW The Dynamic Dependency Limitation REVIEW}

This static assumption breaks down in modern \emph{Multi-Agent Based
Simulations} (MABS) and spatial computing environments like Alchemist. In these
contexts, dependencies are often spatial and transient. For instance, an agent
$A$ depends on agent $B$ only while they are within a specific interaction
radius $r$. As agents move, the topology of the dependency graph changes
continuously.

Adapting the static Gibson-Bruck graph to such dynamic topology requires
frequent, expensive graph reconstruction or reverts the system to inefficient
polling strategies. This exposes a critical gap in current DES architectures:
we possess efficient methods for sorting events (FEL) and efficient methods for
tracking static dependencies (Dependency Graphs), but we lack a unified
framework for handling \emph{dynamic, data-driven dependencies} without
manual intervention.

\section{Dependency Management in Parallel and Distributed Simulations}
\label{sec:pdes}

While the Gibson-Bruck algorithm optimizes the \emph{selection} of the next
...
\label{ssec:objective-reactive-des}
...
new execution model must coexist with the expectations, APIs, and invariants of
...
This chapter motivates the reactive approach and frames the problem.
The architectural and implementation details of the proposed solution are
presented in \Cref{ch:reactive-architecture,ch:implementation}, where we
introduce the reactive abstractions employed by the simulator and discuss how
they are integrated in the execution pipeline.
