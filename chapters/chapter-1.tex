...
modifying \emph{Alchemist}, a \emph{stochastic} \emph{discrete-event simulator}
tailored for \ac{MABS} in \emph{pervasive computing} environments. Because it
can model autonomous agents moving and interacting in continuous space, its
causal dependencies are inherently spatial and highly transient. This make
Alchemist the perfect candidate to illustrate the limitation of static,
explicit dependency graphs (\Cref{ssec:ssa_optimisation}) when applied to
dynamic systems and consequently, an ideal ground for the proposed reactive
approach.

This choice provides a realistic baseline, forcing the new execution model to
coexist with the established expectations and invariants of a complex codebase.
Furthermore, it offers an empirical measure of correctness: passing the
pre-existing extensive test suite provides concrete evidence that the reactive
execution preserves the observable behaviour of the original simulator while
completely replacing its internal dependency management strategy.
