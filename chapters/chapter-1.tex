...
As discussed in the previous section, computer simulation means generating the
behaviour of a model as specified by its dynamic definition, starting from an
initial state. Until now, the simulation algorithm assumed to generate and
execute the model in a sequential fashion. However, components of a model could
be independent of one another, and it does not matter if the behaviours of the
two components are generated in strict sequential order. Model components can
run independently for a limited amount of time until, eventually, an event
makes them dependant on one another. In general, when one component may
directly or indirectly influence another component, there is a causal
dependency of events. To guarantee execution correctness, the generation of
...
\emph{causality constraint}, namely when each modular component of a model
processes the events in non-decreasing timestamp order. From this constraint
follows that for \ac{PDES} simulation, some could either strictly avoid
violating the causality constraint, defining what is known as
\emph{Conservative \ac{PDES}} (\Cref{sssec:conservative_pdes}), while other
approaches known as \emph{Speculative} (\Cref{sssec:speculative_pdes}) have an
optimistic approach which temporarily violate the constraint but then will
detect and repair the violations.


With the raising interest in \ac{PDES}

\subsubsection{Conservative Parallel Execution}\label{sssec:conservative_pdes}

\subsubsection{Speculative Execution}\label{sssec:speculative_pdes}

\subsection{Approximation-based Approaches}
\subsubsection{$\tau$-Leaping}
