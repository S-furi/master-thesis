...
change in the next time step as the cells whose neighbouring cells have not
...
systems. Therefore, simulations utilise numerical integration methods (e.g.
...
\begin{enumerate}
	...
	\item The execution may generate new events or cancel or update existing
		ones by updating the contents of the \ac{FEL};
...
\ac{SSA} (summarised in \Cref{alg:ssa}) ensures correctness by recalculating
the propensity of \emph{all} possible reactions after every single step.
While causally robust, this approach is computationally prohibitive for complex
systems. More in particular:
...
\begin{enumerate}
	...
	\item \emph{Random number reuse}: This property addresses the computational
		  ...
		  random numbers. In the \ac{NRM}, if a reaction $R_j$ is
	      ...
	      putative time $T_j^{new}$ is rescalelgd to maintain statistical
	      exactness:
...
This static assumption breaks down in modern \emph{Multi-Agent Based
Simulations} (\Cref{ssec:mabs}) and spatial computing environments. In these
contexts, dependencies are often spatial and transient and adapting the static
Gibson-Bruck graph to such dynamic topology requires frequent, expensive graph
reconstruction or reverts the system to inefficient polling strategies. More on
this in \Cref{ssec:alchemist_engine}.
...
event by pruning redundant dependency checks, it remains fundamentally tethered
to a \emph{centralised} execution mechanism. In this sequential paradigm, a
single simulation engine maintains the global \ac{FEL} and processes events in
...
common abstraction of \ac{LP} we indicate a computing owning a portion of the
model state, maintaining its local \ac{FEL} while interacting with other
...
allowed to temporarily violate the local causality constraint. Two opposite
approaches have been developed over the years to resolve these distributed
dependencies: \emph{Conservative} and \emph{Speculative} execution.
...
\emph{synchronous message passing}. In this setting, the propagation avents
...
effectively embedding lookahead constraints withing the data flow itself.
...
invalidation} withing a data stream. When an out-of-order event is introduced,
...
\subsubsection{What is a reactive system?}
Reactive systems organise computation around changes: values are observed and
derived values are automatically updated when their sources change. From an
implementation standpoint, this typically manifests as streams (or signals)
that can be subscribed to and as combinators that build higher-level behaviour
by composing these streams. Crucially, the dependency structure is not encoded
as a separate artefact to be manually maintained; it is \emph{implicitly}
represented by the set of active subscriptions created by the running program.
...
%
\begin{itemize}
	\item \textbf{Reduction of cognitive load (developer productivity).} In
	...
	\item \textbf{Consistency by construction.} If state-derived computations
	...
	\item \textbf{Efficiency in high-density systems.} In high-density
...
concrete and assessable, we pursue this objective by modifying an existing
